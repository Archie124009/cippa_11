...
class _TaskState extends State<Task> {
  ...
  final List<List<String>> _answers = [['startDatetime', 'answerDatetime', 'task', 'question', 'answer', 'rightAnswer']];
  DateTime _startTime = DateTime.now();

  // int _currentTask = 0; // Счетчик заданий
  int _currentText = 0; // Для следующих тем
  ...
  Future<void> readTaskData() async {
    final String response = await rootBundle.loadString('assets/data/quiz-Space.data.json');
    final data = await jsonDecode(response);

    setState(() {
      _taskData = data['data'];
      _backgroundImagePath =
          _taskData[0]['questions'][_currentQuestion]['backgroundSource'];
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      ...
      playerText.onPlayerComplete.listen((event) {
        ...
        setState(() {
          _backgroundImagePath =
              _taskData[0]['questions'][_currentQuestion]['imageSource'];
          playerQuestion.play(AssetSource(_taskData[_currentText]['questions'][_currentQuestion]['questionSource']));
        });
      });
      ...
      playerAnswer.onPlayerComplete.listen((event) async {
        ...
        await playerQuestion.setSource(AssetSource(_taskData[_currentText]['questions'][_currentQuestion]['questionSource']));
        await playerAnswer.setSource(AssetSource(_taskData[_currentText]['questions'][_currentQuestion]['rightAnswerSource']));
        await playerText.setSource(AssetSource(_taskData[_currentText]['taskSource']));
  ...
  void _repeatQuestion() async {
    ...
    await playerQuestion.play(AssetSource(_taskData[_currentText]['questions'][_currentQuestion]['questionSource']));
  }

  void _checkAnswer(int answerIndex) {
    final answer = _taskData[_currentText]['questions'][_currentQuestion]['answers'][answerIndex];
    final rightAnswer = _taskData[_currentText]['questions'][_currentQuestion]['rightAnswer'];
          ...
          child: Column(
            children: [
              ...
              if (_showVariants) ...[
                Row(
                  children: [
                    const SizedBox(width: 800.0, height: 500),
                    OutlinedButton(
                      ...
                      child: Padding(
                        ...
                        child: Column(
                          ...
                          children: [
                            Text(
                              _taskData[_currentText]['questions'][_currentQuestion]['answers'][0].toString(),
                    ...
                    OutlinedButton(
                      ...
                      child: Padding(
                        ...
                        child: Column(
                          ...
                          children: [
                            Text(
                              _taskData[_currentText]['questions'][_currentQuestion]['answers'][1].toString(),
                              textAlign: TextAlign.center,
                              style: const TextStyle(
                                fontSize: 15.0,
                                color: Colors.black,
                ...
                Row(
                  children: [
                    const SizedBox(width: 800.0, height: 150),
                    OutlinedButton(
